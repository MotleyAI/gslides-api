...
from gslides_api.text import Shape
...
class TestMarkdownReconstruction:
    ...
    def _assert_features_preserved(self, original: str, reconstructed: str):
        ...
        for link_text, url in original_hyperlinks:
            ...
            assert (
                expected_link in reconstructed
        ...
        for strikethrough_text in original_strikethrough:
            ...
            assert (
                expected_strikethrough in reconstructed
        ...
        for bold_italic, bold_only in original_bold_matches:
            ...
            assert (
                expected_bold in reconstructed
        ...
        for italic_text in original_italic:
            ...
            assert (
                expected_italic in reconstructed
        ...
        for code_text in original_code:
            ...
            assert (
                expected_code in reconstructed
        ...
        for bullet_text in original_bullets:
            ...
            assert (
                expected_bullet in reconstructed
    ...
    def test_numbered_vs_bullet_lists(self, api_response_data):
        ...
        numbered_lines = [
            line for line in lines if any(line.strip().startswith(f"{i}. ") for i in range(1, 10))
        ]

        assert (
            len(bullet_lines) >= 3
        ), f"Should have at least 3 bullet items, got {len(bullet_lines)}"
        assert (
            len(numbered_lines) >= 3
        ), f"Should have at least 3 numbered items, got {len(numbered_lines)}"
...
class TestMarkdownReconstructionEdgeCases:
    ...
    def test_empty_shape(self):
        ...
        shape_element = ShapeElement(
            ...
            shape=Shape(shapeType="TEXT_BOX", shapeProperties=ShapeProperties()),
    ...
    def test_shape_without_text(self):
        ...
        shape_element = ShapeElement(
            ...
            transform={"translateX": 0, "translateY": 0, "scaleX": 1, "scaleY": 1, "unit": "EMU"},
            shape=Shape(shapeType="TEXT_BOX", text=None, shapeProperties=ShapeProperties()),
            presentation_id="test",
        )

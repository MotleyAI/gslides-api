...
from gslides_api.table import Table, TableColumnProperties, TableRow, TableCell, TableCellLocation
...
from gslides_api.request.domain import TableCellLocation as RequestTableCellLocation
...
class TestTableElementResize:
    ...
    def test_resize_requests_no_column_width_info(self):
        ...
        table_element = TableElement(
            objectId="test-table-id",
            size=Size(width=Dimension(magnitude=400, unit=Unit.PT), 
                     height=Dimension(magnitude=300, unit=Unit.PT)),
            transform=Transform(scaleX=1.0, scaleY=1.0, translateX=0.0, translateY=0.0, unit="EMU"),
            table=table,
            slide_id="test-slide-id",
        ...
        update_requests = [r for r in requests_add + requests_delete if isinstance(r, UpdateTableColumnPropertiesRequest)]
    ...
    def test_resize_requests_mixed_changes(self):
        ...
        row_requests = [r for r in requests if isinstance(r, (InsertTableRowsRequest, DeleteTableRowRequest))]
        col_requests = [r for r in requests if isinstance(r, (InsertTableColumnsRequest, DeleteTableColumnRequest))]
        update_requests = [r for r in requests if isinstance(r, UpdateTableColumnPropertiesRequest)]
    ...
    def test_proportional_width_calculation_edge_cases(self):
        ...
        table_element = TableElement(
            objectId="test-table-id",
            size=Size(width=Dimension(magnitude=500, unit=Unit.PT), 
                     height=Dimension(magnitude=300, unit=Unit.PT)),
            transform=Transform(scaleX=1.0, scaleY=1.0, translateX=0.0, translateY=0.0, unit="EMU"),
            table=table,
            slide_id="test-slide-id",
